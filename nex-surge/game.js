...
if (!playerName) {
  playerName = prompt("Enter your nickname:")?.trim() || "anon";
...
function saveScore(name, score) {
  ...
  const globalMax = Math.max(...board.map(e => e.score));
  localStorage.setItem("globalMaxScore", globalMax);
}

function showLeaderboard() {
  const board = JSON.parse(localStorage.getItem("leaderboard") || "[]");
  const list = board.map((e, i) => `${i + 1}. ${e.name}: ${e.score}`).join("\n");
  alert("🏆 Leaderboard:\n" + list);
}
...
lbButton.className = "menu-button";
lbButton.onclick = showLeaderboard;
document.getElementById("menu").appendChild(lbButton);
